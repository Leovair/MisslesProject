...
def fire_missile(x, y):
    info_about_rocket = Missile(color='white', x=BASE_X, y=BASE_Y + 30, x2=x, y2=y)
...
def fire_enemy_missile():
    ...
    if alive_buildings:
        ...
        info_about_rocket = Missile(color='red', x=x, y=y, x2=target.x, y2=target.y)
...
# Проверяекм сколько врагов на экране
def check_enemy_count():
    if len(enemy_missiles_rocket) < ENEMY_COUNT:
        fire_enemy_missile()


# Вычисляем и проверяем столкнулась наша ракета с вражеской
...
def game():
    global our_missiles, enemy_missiles_rocket, buildings, base
